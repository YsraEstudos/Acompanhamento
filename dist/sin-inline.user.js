...
(function() {
	...
	function decodeHttpText(buffer, contentType = "") {
		...
		const [primaryCharset = "utf-8", ...fallbacks] = Array.from(new Set([
			headerCharset,
			metaCharset,
			"utf-8",
			"windows-1252",
		...
		if (!primary) return new TextDecoder("utf-8", { fatal: false }).decode(bytes);
		if (primary.score === 0 || fallbacks.length === 0) return primary.text;
	...
	function detectKlassmattErrorPage(doc) {
		...
		if (/Erro\.aspx/i.test(formAction)) {
			const descriptionText = doc.querySelector("#DivDescricao")?.textContent?.trim() || null;
		...
		if (errorDiv) {
			...
			if (/ACESSO\s+N[ÃA]O\s+AUTORIZADO|exce[çc][ãa]o\s+durante|exception/i.test(text)) return {
				isError: true,
	...
	function extractHistoryIdentityFromUrl(rawUrl, baseUrl = window.location.href) {
		...
		try {
			...
			if (!/\/Historico\.aspx$/i.test(absolute.pathname)) return null;
	...
	function extractHistoryIdentityFromDocument(doc, baseUrl = window.location.href) {
		for (const form of doc.querySelectorAll("form[action]")) {
			const identity = extractHistoryIdentityFromUrl(form.getAttribute("action"), baseUrl);
	...
	function formatHistoryIdentity(identity) {
		...
		].filter(Boolean).join(", ") || identity.absoluteUrl;
	}
	function validateHistoryIdentity(expected, actual) {
		...
		if (expected.pathname.toLowerCase() !== actual.pathname.toLowerCase()) reasons.push(`Caminho inesperado no retorno: esperado ${expected.pathname}, recebido ${actual.pathname}.`);
		if (expected.origin.toLowerCase() !== actual.origin.toLowerCase()) reasons.push(`Origem divergente: esperado ${expected.origin}, recebido ${actual.origin}.`);
		if (!actual.id) reasons.push("A resposta do historico nao informa o Id da SIN no form[action].");
		else if (!expected.id) reasons.push("O link nativo do acompanhamento nao informa o Id da SIN.");
		else if (expected.id !== actual.id) reasons.push(`Id divergente: esperado ${expected.id}, recebido ${actual.id}.`);
		if (expected.source && !actual.source) reasons.push(`A resposta do historico nao informa source=${expected.source}.`);
		else if (expected.source && actual.source && expected.source.toLowerCase() !== actual.source.toLowerCase()) reasons.push(`Source divergente: esperado ${expected.source}, recebido ${actual.source}.`);
		if (expected.somenteLeitura && !actual.somenteLeitura) reasons.push(`A resposta do historico nao informa SomenteLeitura=${expected.somenteLeitura}.`);
		else if (expected.somenteLeitura && actual.somenteLeitura && expected.somenteLeitura !== actual.somenteLeitura) reasons.push(`SomenteLeitura divergente: esperado ${expected.somenteLeitura}, recebido ${actual.somenteLeitura}.`);
		if (expected.k && !actual.k) reasons.push("A resposta do historico perdeu o parametro de seguranca k.");
		else if (expected.k && actual.k && expected.k !== actual.k) reasons.push("O parametro de seguranca k retornou diferente do link nativo.");
		return {
			isValid: reasons.length === 0,
			reasons
		};
	}
	var VALID_NCM_PREFIXES = [
		"0101",
		"0102",
		"0103",
		"0104",
	...
	var VALID_NCM_PREFIX_SET = new Set(VALID_NCM_PREFIXES);
	var VALID_NBS_PREFIX_SET = new Set(VALID_NBS_PREFIXES);
	...
	function detectAttentionMatches(description, yellowComments) {
		...
		for (const match of normalizedCombined.matchAll(/\blei\b/g)) matches.add(match[0].toUpperCase());
		for (const match of rawCombined.matchAll(/(ncm|nbs)?\s*[:=.-]?\s*(\d[\d.\s/-]{6,}\d)/gi)) {
			...
			if (!label && codeIndex >= 0 && hasCaseReferencePrefix(rawCombined, codeIndex)) continue;
			if (label === "NBS") {
	...
	function resolveNearestItemIds(markers, timelineLength) {
		if (timelineLength <= 0) return [];
		const markerByIndex = new Map();
		for (const marker of markers) if (marker.index >= 0 && marker.index < timelineLength) markerByIndex.set(marker.index, marker);
		const nearestLeft = Array(timelineLength);
		let leftMarker;
		...
		const nearestRight = Array(timelineLength);
		let rightMarker;
	...
	function extractCreatedItemId(event) {
		return normalizeTextNoAccent(event.descricao).match(/\bcriado\s+o\s+item\s+n\W*(\d{3,})\b/i)?.[1] || null;
	}
	...
	function scopeTimelineToItem(timeline, currentItemId) {
		...
		if (!normalizedItemId || timeline.length === 0) return {
			status: "unscoped",
			timeline,
			summary: baseSummary,
			detectedItemIds: []
		...
		if (markers.filter((marker) => marker.itemId === normalizedItemId).length === 0) return {
			status: "ambiguous",
			timeline,
			summary: baseSummary,
			detectedItemIds,
		...
		if (bestSegment.length === timeline.length) return {
			status: "unscoped",
			timeline,
			summary: baseSummary,
			detectedItemIds
	...
	function isSignificantNode(node) {
		...
		if (node.nodeType === Node.TEXT_NODE) return Boolean(normalizeSpaces(node.textContent || ""));
		if (node instanceof Element) return Boolean(normalizeSpaces(node.textContent || ""));
		return false;
	}
	function getPreviousSignificantNode(nodes, index) {
		for (let cursor = index - 1; cursor >= 0; cursor--) if (isSignificantNode(nodes[cursor])) return nodes[cursor];
		return null;
	}
	function getNextSignificantNode(nodes, index) {
		for (let cursor = index + 1; cursor < nodes.length; cursor++) if (isSignificantNode(nodes[cursor])) return nodes[cursor];
		return null;
	}
	...
	function resolveYellowNoteContainer(element) {
		...
		if (!text) return null;
		if (isYellowHighlightElement(element)) return element;
	...
	function removeAcceptedNotes(root, acceptedNotes) {
		...
		for (const node of unique) node.remove();
		while (root.firstChild instanceof HTMLBRElement) root.firstChild.remove();
		while (root.lastChild instanceof HTMLBRElement) root.lastChild.remove();
	}
		...
		for (const fieldset of doc.querySelectorAll("fieldset.hist-fieldset")) {
			...
			for (const row of fieldset.querySelectorAll(".row")) {
				if (!row.classList.contains("result")) {
					...
					if (userLink) currentUser = normalizeSpaces(userLink.textContent || "").replace(/\*+$/, "");
					continue;
				}
	...
	var PLAIN_URL_PATTERN = /https?:\/\/[^\s<>()"']+/gi;
	var SNAPSHOT_PASSTHROUGH_TAGS = new Set([
		"ARTICLE",
		"B",
		"BLOCKQUOTE",
		"BR",
	...
	function sanitizeNode(node, baseUrl, options) {
		if (node.nodeType === Node.TEXT_NODE) return sanitizeTextNode(node.textContent ?? "", baseUrl);
		...
		if (tag === "A") {
			...
			if (!text) return null;
			if (!href || /^javascript:/i.test(href)) return document.createTextNode(text);
		...
		if (!options.passthroughTags.has(tag)) return fragment;
	...
	function sanitizeInlineHtml(value, baseUrl = window.location.href) {
		...
		for (const child of template.content.childNodes) {
			const sanitized = sanitizeNode(child, baseUrl, { passthroughTags: INLINE_PASSTHROUGH_TAGS });
	...
	function sanitizeSnapshotHtml(value, baseUrl = window.location.href) {
		...
		for (const child of root.childNodes) {
			const sanitized = sanitizeNode(child, baseUrl, { passthroughTags: SNAPSHOT_PASSTHROUGH_TAGS });
			if (sanitized) container.appendChild(sanitized);
		}
		return container.innerHTML;
	}
	var STYLE_ID$1 = "km-sin-sidebar-style";
	var LAYOUT_SELECTOR = ".km-sin-layout[data-km-sin-root=\"1\"]";
...
<html lang="pt-BR">
  ...
  <body>
    ...
    <main class="km-sin-snapshot">${(rawHtml ? sanitizeSnapshotHtml(rawHtml, baseUrl) : `<p>Nenhum snapshot seguro estava disponivel para este historico.</p><p><a href="${escapeHtml(historyUrl)}" target="_blank" rel="noreferrer noopener">Abrir historico nativo em nova aba</a></p>`) || "<p>O historico nao trouxe conteudo visual seguro para exibir inline.</p>"}</main>
  </body>
</html>`;
	}
	...
	function ensureShell(viewRoot) {
		let existing = null;
		for (const child of viewRoot.children) if (child instanceof HTMLElement && child.matches(LAYOUT_SELECTOR)) {
			existing = child;
			break;
		}
		if (existing) {
			const mainEl = existing.querySelector(".km-sin-main");
			const asideEl = existing.querySelector(".km-sin-aside");
			const metaEl = existing.querySelector(".km-sin-meta");
			const stateEl = existing.querySelector(".km-sin-state");
			const bodyEl = existing.querySelector(".km-sin-body");
			const inlineButton = existing.querySelector("[data-role=\"inline\"]");
			const modeButton = existing.querySelector("[data-role=\"mode\"]");
			if (mainEl && asideEl && metaEl && stateEl && bodyEl && inlineButton && modeButton) return {
				layoutEl: existing,
				mainEl,
				asideEl,
				metaEl,
  ...
  `;
		...
		desc.innerHTML = (event.descricaoHtml ? sanitizeInlineHtml(event.descricaoHtml, historyUrl) : sanitizeInlineHtml(event.descricao, historyUrl)) || escapeHtml(event.descricao);
	...
	function appendTimeline(shell, model) {
		shell.bodyEl.querySelector("[data-act=\"load-more\"]")?.parentElement?.remove();
		const lastGroup = Array.from(shell.bodyEl.querySelectorAll(".km-sin-group")).at(-1) || null;
		let currentDay = lastGroup?.querySelector(".km-sin-day")?.textContent || "";
		let list = lastGroup?.querySelector(".km-sin-list") || null;
		for (const event of model.timeline) {
		...
		if (model.onLoadMore && typeof model.totalCount === "number" && typeof model.loadedCount === "number" && model.loadedCount < model.totalCount) {
			const actions = document.createElement("div");
			actions.className = "km-sin-empty-actions";
			const button = document.createElement("button");
			button.type = "button";
	...
	var SinSidebarApp = class {
		...
		applySettings(nextSettings) {
			...
			if (alwaysOpenChanged) this.inlinePanelOverride = null;
		...
		async hydrate(force = false) {
			...
			setShellMeta(shell, confirmedContext.sinId ? `SIN ${confirmedContext.sinId} • historico sob demanda` : "Historico do item carregado sob demanda");
		...
		renderResult(shell, context, result) {
			...
			if (result.mode === "session-error") {
				...
				renderEmpty(shell, [result.diagnostic, result.actionHint].filter(Boolean).join(" ") || "A sessao do Klassmatt expirou. Recarregue a pagina (F5).");
				return;
			}
			if (result.mode === "blocked" && getSafeHistoryUrl(context.historyUrl)) {
				...
				renderIframeFallbackPrompt(shell, result.diagnostic, () => {
					...
					renderIframeFallback(shell, safeHistoryUrl, result.diagnostic, result.inlineHtml, result.inlineBaseUrl);
				});
				return;
			}
		...
		renderStoredTimeline(shell, appendFrom) {
			...
			if (this.renderedCount === 0) this.renderedCount = Math.min(RENDER_BATCH_SIZE, visibleTimeline.length);
			else this.renderedCount = Math.min(this.renderedCount, visibleTimeline.length);
			const renderedTimeline = appendFrom === void 0 ? visibleTimeline.slice(0, this.renderedCount) : visibleTimeline.slice(appendFrom, this.renderedCount);
			const loadedCount = appendFrom === void 0 ? renderedTimeline.length : this.renderedCount;
		...
		resolveConnectedShell() {
			this.pruneDisconnectedShell();
			if (this.currentShell?.layoutEl.isConnected) return this.currentShell;
		...
		getContextScopeKey(context) {
			...
			return [context.itemId || "sem-item", identityScope || "sem-contexto"].join("|");
		}
		syncContextScope(context) {
			const nextContextKey = this.getContextScopeKey(context);
			if (!nextContextKey) return;
			if (this.currentContextKey && this.currentContextKey !== nextContextKey) this.inlinePanelOverride = null;
		...
		syncInlineToggle(_context = resolveQuickPageContext()) {
			...
			if (!this.toggleHost || !this.toggleButton || !this.toggleHost.isConnected || this.toggleParent !== parent) {
				this.removeInlineToggle();
			...
			if (toggleHost.previousElementSibling !== linkEl) linkEl.insertAdjacentElement("afterend", toggleHost);
		...
		removeInlineToggle() {
			if (this.toggleHost?.isConnected) this.toggleHost.remove();
		...
		syncClosedState(context = resolveQuickPageContext()) {
			...
			if (!this.panelOpen) this.hideCurrentSidebar();
		}
		closePanel() {
			this.panelOpen = false;
			this.loadSerial++;
		...
		hideCurrentSidebar(clearBody = false) {
			if (this.currentShell?.layoutEl.isConnected) {
				if (clearBody) this.currentShell.bodyEl.replaceChildren();
		...
		buildTimelineSummary(loadedCount, totalVisible) {
			...
			if (this.settings.timelineMode === "yellow-only") return loadedCount < totalVisible ? `Exibindo ${loadedCount} de ${totalVisible} evento(s) com comentario amarelo` : `Exibindo ${totalVisible} evento(s) com comentario amarelo`;
	...
	var CONTEXT_HINT_SELECTOR = "#UpdatePanel1, .kl-view, #DV_Resumo_sin, #hlkObs, #hButAcompanhamentoSIN";
	...
	var STYLE_ID = "km-unspsc-quick-style";
	var QUICK_SELECTOR = "[data-km-unspsc-quick=\"1\"]";
	var OWNED_SELECTOR = `${QUICK_SELECTOR}, [data-km-unspsc-toast="1"]`;
	var DEFAULT_TIMEOUT_MS = 15e3;
	var DEFAULT_AUTO_SUBMIT_DELAY_MS = 180;
	var PENDING_KEY = "km_unspsc_pending_v1";
	var SELECTORS = {
	...
	function findNativeUnspscElements() {
		...
		for (const value of values) {
			...
			const lookup = (value.parentElement ?? document).querySelector(SELECTORS.nativeLookup) ?? document.querySelector(SELECTORS.nativeLookup);
			if (lookup && isUsableElement(lookup)) return {
				value,
				lookup
			};
		}
	...
	function findUnspscModal() {
		const codeInputs = Array.from(document.querySelectorAll(SELECTORS.modalCode));
		for (const codeInput of codeInputs) {
			const table = codeInput.closest("table");
			if (table && isUsableElement(codeInput) && table.querySelector(SELECTORS.modalSearch) && table.querySelector(SELECTORS.modalClose)) return table;
		}
		return null;
	}
	...
	function writePendingUnspsc(pending) {
		try {
			sessionStorage.setItem(PENDING_KEY, JSON.stringify(pending));
		} catch {}
	}
	function clearPendingUnspsc() {
		try {
	...
	function findExactResult(code) {
		...
		for (const row of grid.querySelectorAll("tr")) {
			if (normalizeUnspscCode(Array.from(row.querySelectorAll("a, span, td")).find((element) => element.id === "lbCodigo" || /\$lbCodigo$/.test(element.getAttribute("name") || ""))?.textContent || "") !== code) continue;
			const selector = row.querySelector("input[name$=\"$ckSelUNSPSC\"], input[id$=\"ckSelUNSPSC\"]");
	...
	var UnspscQuickFillApp = class {
		...
		sync() {
			...
			if (!existingHost || !existingHost.isConnected || this.nativeValue !== native.value) {
			...
			if (this.codeInput && document.activeElement !== this.codeInput) this.codeInput.value = this.running ? this.activeCode : extractCurrentCode(native.value.value);
			this.renderState();
		}
		...
		handleInput = (event) => {
			...
			if (this.autoSubmitTimer) window.clearTimeout(this.autoSubmitTimer);
			if (code.length !== 8) {
				this.setState(code.length > 8 ? "Código deve ter 8 dígitos." : "Digite os 8 dígitos.", code.length > 8 ? "error" : "idle");
				return;
			}
		...
		handleKeyDown = (event) => {
			...
			if (code.length === 8) this.startFill(code);
		};
		handleBlur = (event) => {
			const code = normalizeUnspscCode(event.currentTarget.value);
		...
		async cancelModal(serial) {
			const cancel = findUnspscModal()?.querySelector(SELECTORS.modalCancel);
		...
		renderState() {
			if (this.codeInput && this.codeInput.disabled !== this.running) this.codeInput.disabled = this.running;
			if (this.status) {
				if (this.status.textContent !== this.statusMessage) this.status.textContent = this.statusMessage;
				if (this.status.dataset.tone !== this.statusTone) this.status.dataset.tone = this.statusTone;
			}
			const toast = this.ensureToast();
			if (toast.textContent !== this.statusMessage) toast.textContent = this.statusMessage;
			const nextHidden = !this.running;
			if (toast.hidden !== nextHidden) toast.hidden = nextHidden;
		}
		ensureToast() {
	...
	var app = null;
	var unspscQuickFill = null;
	var alwaysOpenMenuId = null;
	var bootstrapObserver = null;
	var bootstrapCleanupTimer = 0;
	var BOOTSTRAP_OBSERVER_TIMEOUT_MS = 15e3;
