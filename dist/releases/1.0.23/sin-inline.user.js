...
<html lang="pt-BR">
  ...
  function findUnspscModal() {
    ...
    for (const codeInput of codeInputs) {
      ...
      if (table && isUsableElement(codeInput) && table.querySelector(SELECTORS.modalSearch) && table.querySelector(SELECTORS.modalClose)) {
        return table;
      }
    }
    return null;
  ...
  function readPendingUnspsc() {
    try {
      ...
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      return parsed.code && parsed.code.length === 8 && parsed.stage ? { code: parsed.code, stage: parsed.stage } : null;
    } catch {
      return null;
    }
  }
